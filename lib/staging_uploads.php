...
function validate_form_upload_file(array $upload, array $field, int $maxBytes, array $allowedMimes): array
{
    ...
    if (($upload['error'] ?? UPLOAD_ERR_NO_FILE) !== UPLOAD_ERR_OK) {
        return ['ok' => false, 'error' => 'Upload failed for ' . $label . '.'];
    }
    if ((int) ($upload['size'] ?? 0) > $maxBytes) {
        return ['ok' => false, 'error' => $label . ' exceeds max file size.'];
    }

    $finfo = new finfo(FILEINFO_MIME_TYPE);
    $mime = $finfo->file($upload['tmp_name']) ?: ($upload['type'] ?? 'application/octet-stream');
    if ($type === 'image' && !str_starts_with($mime, 'image/')) {
        return ['ok' => false, 'error' => $label . ' must be an image.'];
    }
    if ($allowedMimes && !in_array($mime, $allowedMimes, true)) {
        return ['ok' => false, 'error' => $label . ' file type is not allowed.'];
    }
